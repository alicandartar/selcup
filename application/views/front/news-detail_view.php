		...
		<div class="wrap">
					...
					<ul>
						<?php foreach ($menus as $menu) { ?>
							<?php if ($menu->is_submenu == 1) { ?>
								<?php echo "<li><a href='".base_url()."product/".$menu->slug."'>$menu->menu_name<i class='fas fa-chevron-down'></i></a>";?>
								<?php echo "<div class='product-submenu'>";?>
								<?php echo "<div class='product-submenu-list'>";?>
								<?php foreach ($submenus as $submenu) { 
									echo "<div class='item'>";
									 echo "<a href='".base_url()."product/".$submenu->slug."'>";
								...
								<?php } else { ?>
									<?php if($menu->slug == "sustainability") {echo "<li class='sustainability'><a href='".base_url().$menu->slug."'>$menu->menu_name</a></li>";} else {?>
										<?php if($this->uri->segment(1) == $menu->slug){ echo "<li><a class='active' href='".base_url().$menu->slug."'>$menu->menu_name</a></li>";?>
			...
			<div class="contact-section hidden">
				<div class="contact-content">
					<div class="menu">
						<?php foreach ($submenus as $submenu) {?>
							<?php echo '<a href="'.base_url().'product/'.$submenu->slug.'">'.$submenu->sub_menu_name.'</a>'; ?>
						<?php }?>
					</div>
					<div class="item">
						...
						<ul>
							<?php foreach ($menus as $menu) { ?>
								<?php if ($menu->is_submenu == 1) { ?>
									...
									<ul>
										<?php foreach ($submenus as $submenu) { echo "<li><a href='".base_url()."product/".$submenu->slug."'>$submenu->sub_menu_name</a></li>"; } ?>
									</ul>
									<?php } else { if($menu->slug == 'main'){echo "<li><a href='".base_url()."'>$menu->menu_name</a></li>";}else{echo "<li><a href='".base_url().$menu->slug."'>$menu->menu_name</a></li>";} } ?>					
							<?php } ?>
						</ul>
			...
			<section>
				...
				<div class="page-title">
					...
					<div class="container">
						<h1><?php echo $newsdetail[0]->title; ?></h1>
					</div>
				</div>
				...
				<div class="news content-start">
					<div class="container">
						<div class="news-date">
							<span><?php echo date("d/m/Y", $newsdetail[0]->date); ?></span>
						</div>
						<div class="static-content-text">
							<?php echo $newsdetail[0]->detail; ?>
						</div>
					</div>
				</div>
							...
							<div class="row">
								...
								<div class="menu">
										<ul>
											<?php foreach ($menus as $menu) { ?>
												<?php if ($menu->is_submenu == 1) { ?>
													<?php echo "<li><a href='".base_url()."product/".$menu->slug."'>$menu->menu_name</a>";?>
													<ul>
														<?php foreach ($submenus as $submenu) { echo "<li><a href='".base_url()."product/".$submenu->slug."'>$submenu->sub_menu_name</a></li>"; } ?>
													</ul>
													<?php } else { if($menu->slug == 'main'){echo "<li><a href='".base_url()."'>$menu->menu_name</a></li>";}else{echo "<li><a href='".base_url().$menu->slug."'>$menu->menu_name</a></li>";} } ?>					
