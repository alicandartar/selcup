...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        $('#CopyImage').click(function(){
            ...
            var x = $('.clone').length;
            var clone = '<div class="GalleryImage clone"><div class="col-md-2 table-bordered"><div class="form-group"><label>Başlık</label><input type="text" class="form-control" name="title[]"></div><div class="form-group"><label>İcon</label><div class="input-group"><input type="hidden" class="form-control" name="images[]" id="SliderImage'+x+'"><a href="<?=base_url()?>filemanager/dialog.php?type=1&field_id=SliderImage'+x+'&relative_url=1&akey=Qxa48BPNzyw0tk28c2QktofjTzyW8e3t" class="btn btn-default btn-flat iframe-btn"><i class="fa fa-search"> Dosya Seç</i></a><img id="Image'+x+'" src="http://placehold.it/507x250&text=Image Area" class="img-responsive"></div></div><div class="form-group"><label>Durumu</label><select name="status[]" class="form-control"><option value="1">Aktif</option><option value="0">Pasif</option></select></div><div class="form-group"><label>Sıra</label><input type="number" class="form-control" name="sort_sequence[]" value="'+x+'"></div></div></div>';
                                ...
                                <div class="col-md-2 table-bordered">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="icon" class="form-control">
                                            <option value="facebook" <?=$info->icon == 'facebook' ? 'selected' : NULL; ?>>Facebook</option>
                                            <option value="twitter" <?=$info->icon == 'twitter' ? 'selected' : NULL; ?>>Twitter</option>
                                            <option value="linkedin" <?=$info->icon == 'linkedin' ? 'selected' : NULL; ?>>Linkedin</option>
                                            <option value="google-plus" <?=$info->icon == 'google-plus' ? 'selected' : NULL; ?>>Google Plus</option>
                                            <option value="youtube" <?=$info->icon == 'youtube' ? 'selected' : NULL; ?>>Youtube</option>
                                            <option value="instagram" <?=$info->icon == 'instagram' ? 'selected' : NULL; ?>>Instagram</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>Durumu</label>
                                        <input type="number" class="form-control" name="sort_sequence" value="<?=$info->sort_sequence?>">
                                    </div>
                                </div>
                            </div>
                        </div>
