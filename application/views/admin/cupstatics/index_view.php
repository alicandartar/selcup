...
<script type="text/javascript" charset="utf-8">
    $( document ).ready(function() {
        ...
        $("#ListDelete").on("click",function(){
            ...
            $.ajax({
                url: "<?php echo base_url(); ?>admin/cupstatics/ListDelete",
                type: "post",
                data: data,
                dataType: "text",
                cache: false,
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1>
            <small>
                <a class="btn btn-app" href="<?=base_url()?>admin/features">
                    <span class="badge bg-teal"><?=$TotalPage?></span>
                    <i class="fa fa-tags"></i> İçerik
                </a>
            </small>
            <small>
                <a class="btn btn-app" href="<?=base_url()?>admin/cupstatics/add">
                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-hover">
                            ...
                            <tbody>
                            <?php foreach($cupstatics as $key => $cupstatic){?>
                                <tr>
                                    <td><input type="checkbox" id="ID" class="ID" value="<?=$cupstatic->id?>" name="ID[]"> </td>
                                    <td><?php foreach($cupsname as $cupname){if($cupname->id == $cupstatic->cups_name_id){echo $cupname->name;}}?></td>
                                    <td><?=$cupstatic->statement?></td>
                                    <td><a href="<?=base_url()?>admin/cupstatics/edit/<?=$cupstatic->id?>" class="btn btn-warning">Düzenle</a></td>
                                </tr>
                            <?php }?>
                            </tbody>
                            <tfoot>
