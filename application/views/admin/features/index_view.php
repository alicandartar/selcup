...
<script type="text/javascript" charset="utf-8">
    $( document ).ready(function() {
        ...
        $("#ListDelete").on("click",function(){
            ...
            $.ajax({
                url: "<?php echo base_url(); ?>admin/features/ListDelete",
                type: "post",
                data: data,
                dataType: "text",
                cache: false,
                    ...
                    <div class="box-body">
                        <table id="example1" class="table table-bordered table-hover">
                            ...
                            <?php foreach($features as $key => $feature){?>
                                <tr>
                                    <td><input type="checkbox" id="ID" class="ID" value="<?=$feature->id?>" name="ID[]"> </td>
                                    <td><?=$feature->title?></td>
                                    <td><?=$feature->content?></td>
                                    <td><img src="<?=base_url().$feature->image?>" width="100"> </td>
                                    <td><a href="<?=base_url()?>admin/features/edit/<?=$feature->id?>" class="btn btn-warning">Düzenle</a></td>
                                </tr>
                            <?php }?>
                            </tbody>
                            <tfoot>
